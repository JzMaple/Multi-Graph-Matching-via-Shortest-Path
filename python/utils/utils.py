...
def padding(x, shape):
  ...
  device = x.device
  for i, (s1, s2) in enumerate(zip(x_shape, shape)):
    assert s1 >= s2
    if s1 < s2:
      y_shape = list(x.shape)
      y_shape[i] = s2 - s1
      y = torch.zeros(y_shape).to(device)
      x = torch.cat([x, y], dim=i)
  return x

def list2tensor(l):
  shape_list = []
  for x in l:
    x_shape = torch.tensor(x.shape).reshape(1, -1)
    shape_list.append(x_shape)
  shape_list = torch.cat(shape_list, dim=0)
  shape_max, _ = torch.max(shape_list, dim=0)
  shape_max = list(shape_max)
  ans = []
  for x in l:
    ans.append(padding(x, shape_max).unsqueeze(0))
  ans = torch.cat(ans, dim=0)
  return ans

def lexlist2tensor(l, m, diag):
  for x in l: assert x.size() == l[0].size()
  ans_shape = torch.Size((m, m)) + l[0][0].size()
  ans = torch.zeros(ans_shape).to(l[0].device)
  for e, (i, j) in enumerate(lexico_iter(range(m))):
    ans[i, j] = l[e][0]
    ans[j, i] = l[e][0].transpose(0, 1)
  for i in range(m):
    ans[i, i] = diag
  return ans
